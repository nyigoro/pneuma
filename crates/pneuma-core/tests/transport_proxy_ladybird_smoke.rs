#![cfg(all(feature = "ladybird", feature = "transport-stealth"))]
...
#[ignore = "requires ladybird + transport-stealth features and LADYBIRD_BUILD_DIR"]
async fn transport_proxy_is_honored_by_lazy_ladybird_launch() -> Result<()> {
    if std::env::var("LADYBIRD_BUILD_DIR").is_err() {
        eprintln!("[transport-proxy-ladybird] skipping: LADYBIRD_BUILD_DIR not set");
        return Ok(());
    }
    ...
    let template = pneuma_broker::LaunchTemplate {
        kind: pneuma_engines::EngineKind::Ladybird,
        stealth: true,
    ...
    let nav_joined = tokio::time::timeout(Duration::from_secs(45), nav_task)
        .await
    ...
    if let Err(error) = nav_outcome {
        ...
        {
            eprintln!(
                "[transport-proxy-ladybird] skipping: Ladybird startup unavailable in this environment: {message}"
            );
    ...
    let payload = tokio::time::timeout(Duration::from_secs(15), seen_rx)
        .await
        .context("recording proxy did not receive any traffic from Ladybird engine")?
